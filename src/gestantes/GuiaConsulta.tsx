...
type MetodoBCF = 'nao_informado' | 'sonar_doppler' | 'pinard'
const ROTULO_METODO_BCF: Record<MetodoBCF, string> = { nao_informado: '', sonar_doppler: 'sonar Doppler', pinard: 'Pinard' }

type MovimentacaoFetal = 'nao_avaliado' | 'presente' | 'reduzida' | 'ausente'
 ...
 *  Nada persiste — reseta ao trocar de aba/recarregar, é a paciente na cadeira agora. */
export function GuiaConsulta() {
  ...
  const [historiaObstetrica, setHistoriaObstetrica] = useState('')
  const [tipoSanguineo, setTipoSanguineo] = useState('')
  const [comorbidades, setComorbidades] = useState('')
  const [medsSelecionados, setMedsSelecionados] = useState<Set<string>>(new Set())
  const [outrosMedicamentos, setOutrosMedicamentos] = useState('')
  const [alergias, setAlergias] = useState('')
  const [negaVicios, setNegaVicios] = useState(true)
  const [viciosDetalhe, setViciosDetalhe] = useState('')
  const [toxoplasmose, setToxoplasmose] = useState<StatusSorologia>('desconhecido')
  const [atividadeLaboral, setAtividadeLaboral] = useState('')

  // ---- subjetivo ----
  const [acompanhada, setAcompanhada] = useState(false)
  const [acompanhantePor, setAcompanhantePor] = useState('')
  const [encaminhadaPor, setEncaminhadaPor] = useState('')
  const [jaRealizouExames, setJaRealizouExames] = useState(false)
  const [queixas, setQueixas] = useState<Record<string, boolean>>({})
  const [queixasDetalhe, setQueixasDetalhe] = useState('')
  const [vacinasTomadas, setVacinasTomadas] = useState<Set<string>>(new Set())

  // ---- objetivo / exame físico ----
  const [peso, setPeso] = useState('')
  const [estatura, setEstatura] = useState('')
  const [abdomeGravidico, setAbdomeGravidico] = useState(false)
  const [au, setAu] = useState('')
  const [bcfBpm, setBcfBpm] = useState('')
  const [bcfMetodo, setBcfMetodo] = useState<MetodoBCF>('nao_informado')
  const [bcfAusente, setBcfAusente] = useState(false)
  const [movimentacaoFetal, setMovimentacaoFetal] = useState<MovimentacaoFetal>('nao_avaliado')
  const [achados, setAchados] = useState<Set<string>>(new Set())

  // ---- avaliação / plano ----
  const [riscoAlto, setRiscoAlto] = useState(false)
  const [planoExtra, setPlanoExtra] = useState('')
  ...
  const textoFinal = useMemo(() => {
    ...
    const textoQueixas =
      referidas.length === 0
        ...
        : `Refere ${referidas.map((p) => p.titulo.toLowerCase()).join(', ')}${queixasDetalhe ? ' — ' + queixasDetalhe : ''}. Nega demais queixas.`
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {PERGUNTAS_ESSENCIAIS.map((p) => (
                    <ToggleChip
                      key={p.titulo}
                      ativo={!!queixas[p.titulo]}
                      onClick={() => setQueixas((prev) => ({ ...prev, [p.titulo]: !prev[p.titulo] }))}
                      label={p.titulo}
                      alerta={!!queixas[p.titulo]}
                    />
                  ))}
                </div>
