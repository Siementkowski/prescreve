...
import { Search, X, FolderHeart, Pill, ArrowLeft } from 'lucide-react'
import { useConsultaStore } from './store'
import { useSyncStore } from '../core/sync'
import { patologiasDoMedicamento } from './filtros'
...
export function BuscaGlobal() {
  ...
  const [medicamentoAberto, setMedicamentoAberto] = useState<Medicamento | null>(null)
  ...
  const patologiasDoMedicamentoAberto = useMemo(() => {
    if (!medicamentoAberto) return []
    return patologiasDoMedicamento(medicamentoAberto.id, itens, tratamentos, patologiaComplementos, patologias)
  }, [medicamentoAberto, itens, tratamentos, patologiaComplementos, patologias])
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 border-b border-border px-4 py-3">
              {medicamentoAberto ? (
                <button
                  onClick={() => setMedicamentoAberto(null)}
              ...
              {medicamentoAberto ? (
                <span className="flex-1 text-sm font-semibold text-text truncate">{medicamentoAberto.nome}</span>
              ) : (
                <input
                  ref={inputRef}
                  value={termo}
              ...
              {medicamentoAberto ? (
                <div>
                  <p className="text-[11px] font-semibold text-text-dim uppercase tracking-wide px-4 py-1.5">
                    Patologias com {medicamentoAberto.nome}
                  </p>
                  {patologiasDoMedicamentoAberto.length === 0 ? (
                    <p className="text-sm text-text-dim px-4 py-6 text-center">
                      Não encontrado em nenhuma patologia vinculada.
                    </p>
                  ) : (
                    patologiasDoMedicamentoAberto.map((p) => (
                      <button
                        key={p.id}
                  ...
                  {resultadosMedicamentos.length > 0 && (
                    <div>
                      ...
                      {resultadosMedicamentos.map((m) => (
                        <button
                          key={m.id}
                          onClick={() => setMedicamentoAberto(m)}
                          className="w-full flex items-center gap-2.5 text-left px-4 py-2 hover:bg-surface-2 transition-colors"
                        >
